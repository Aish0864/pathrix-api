...
def register(request: RegisterRequest):
    ...
    cursor.execute("SELECT id FROM students WHERE email = %s", (request.email,))
    if cursor.fetchone():
        conn.close()
        raise HTTPException(status_code=400, detail="Email already registered")
...
def login(request: LoginRequest):
    ...
    cursor.execute("SELECT * FROM students WHERE email = %s", (request.email,))
    student = cursor.fetchone()
    conn.close()
...
def submit_quiz(request: SubmitQuizRequest):
    ...
    cursor.execute("""
        UPDATE sessions SET interactions = %s WHERE session_id = %s
    """, (json.dumps(all_interactions), request.session_id))
...
def mark_mastered(request: MarkMasteredRequest):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT mastery_pct FROM student_mastery WHERE student_id = %s AND concept_id = %s",
        (request.student_id, request.concept_id)
    )
    row = cursor.fetchone()
    conn.close()
    mastery_before = row["mastery_pct"] if row else 0.0
...
def get_rewards(student_id: str, limit: int = 20):
    ...
    return [
        {
            "concept": CONCEPT_NAMES[r["concept_id"]] if r["concept_id"] < len(CONCEPT_NAMES) else f"Concept {r['concept_id']}",
            "mastery_before": round(r["mastery_before"], 1),
            "mastery_after":  round(r["mastery_after"], 1),
            "score":          r["score"],
            "total":          r["total"],
            "reward":         r["reward"],
...
def get_admin_students():
    ...
    for student in students:
        cursor.execute(
            "SELECT COUNT(*) as count FROM interactions WHERE user_id = %s",
            (student["id"],)
        )
        interaction_count = cursor.fetchone()["count"]

        cursor.execute(
            "SELECT COUNT(*) as count, MAX(last_recommendation) as last_rec, MAX(confidence) as conf, MAX(cognitive_load) as cog FROM sessions WHERE student_id = %s",
            (student["id"],)
        )
        session_data = cursor.fetchone()
        session_count = session_data["count"]
        last_recommendation = session_data["last_rec"]
        confidence = session_data["conf"]
        cognitive_load = session_data["cog"]

        cursor.execute(
            "SELECT AVG(correct) * 100 as avg FROM interactions WHERE user_id = %s",
            (student["id"],)
        )
        avg_mastery = cursor.fetchone()["avg"] or 0

        cursor.execute(
            "SELECT COUNT(*) as count FROM student_mastery WHERE student_id = %s AND mastered = 1",
            (student["id"],)
        )
        mastered_count = cursor.fetchone()["count"] or 0

        cursor.execute(
            "SELECT MAX(timestamp) as ts FROM interactions WHERE user_id = %s",
            (student["id"],)
        )
        last_active_row = cursor.fetchone()["ts"]
        if last_active_row:
            from datetime import datetime
            last_date = str(last_active_row)[:10]
            today = datetime.now().strftime("%Y-%m-%d")
            last_active = "today" if last_date == today else last_date
        else:
            last_active = "never"
